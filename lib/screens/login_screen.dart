...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_isSignUp) {
      success = await authProvider.registerWithEmail(email, password);
    } else {
      success = await authProvider.loginWithEmail(email, password);
    }
    ...
    if (mounted) {
      ...
      if (!success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                _isSignUp ? 'Inscription réussie !' : 'Connexion réussie !'),
            backgroundColor: const Color(0xff22c55e),
          ),
        );
        // Load profile and merge any onboarding answers saved earlier
        final profileProv =
            Provider.of<ProfileProvider>(context, listen: false);
        await profileProv.loadProfile(
            Provider.of<AuthProvider>(context, listen: false).uid!);
        final profile = profileProv.profile;
        // Merge onboarding answers from SharedPreferences if present
        final prefs = await SharedPreferences.getInstance();
        final activity = prefs.getString('onboarding_activity');
        final problem = prefs.getString('onboarding_productivity_problem');
        final mood = prefs.getString('onboarding_mood');
        if (profile != null &&
            (activity != null || problem != null || mood != null)) {
          ...
          if (problem != null && problem.isNotEmpty)
            merged['productivityProblem'] = problem;
          if (mood != null && mood.isNotEmpty) merged['mood'] = mood;
          merged['onboardingCompleted'] = true;
          final updated = profile.copyWith(preferences: merged);
          await profileProv.updateProfile(updated);
          // clear temp onboarding prefs
          await prefs.remove('onboarding_activity');
          await prefs.remove('onboarding_productivity_problem');
          await prefs.remove('onboarding_mood');
          Provider.of<AppStateProvider>(context, listen: false)
              .completeOnboarding();
        }
        // Load goals and tasks for the user
        try {
        ...
        if (profile != null &&
            profile.preferences['onboardingCompleted'] == true) {
          Provider.of<AppStateProvider>(context, listen: false)
              .completeOnboarding();
        }
      }
  ...
  Future<void> _continueAsGuest() async {
    ...
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    bool success = await authProvider.continueAsGuest();
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.email_outlined,
                          color: const Color.fromRGBO(99, 102, 241, 0.7)),
                      filled: true,
                      fillColor:
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(14)),
                      elevation: 4,
                      shadowColor: const Color.fromRGBO(99, 102, 241, 0.3),
                    ),
                    child: _isLoading
                        ? const SizedBox(
                            height: 20,
