...
app.post("/api/coach", async (req, res) => {
  ...
  try {
    ...
    const goalContext = Array.isArray(goalsSummary) && goalsSummary.length > 0
      ? `- Objectifs actifs : ${goalsSummary.map((goal: any) => `${goal.title ?? 'Objectif'} (${Math.round(((goal.progress ?? 0) as number) * 100)}%)`).join(', ')}`
      : '- Objectifs actifs : aucun objectif enregistré.';

    const progressContext = progressSummary && typeof progressSummary === 'object'
      ? `- Progression : XP ${progressSummary.xp ?? 0}, Niveau ${progressSummary.level ?? 0}, Série ${progressSummary.streak ?? 0}, Tâches ${progressSummary.completedTasksCount ?? 0}/${progressSummary.totalTasksCount ?? 0}, Mission moyenne ${(Math.round(((progressSummary.averageMissionProgress ?? 0) as number) * 100))}%`
    ...
    const prompt = `
      ...
      - Temps de focus total : ${focusMinutesTotal ?? 0} minutes
